...
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, isSameMonth, addMonths, subMonths, parseISO, addDays, startOfDay, startOfWeek, endOfWeek, isPast } from 'date-fns';
...
export function AvailabilityManager({ onAvailabilityChange }: AvailabilityManagerProps) {
  ...
  const getDayClasses = (date: Date) => {
    ...
    if (isDateAvailable(date) && isPast(date) && !isToday(date)) {
      // Día que está HABILITADO pero YA PASÓ (excluyendo hoy): Verde con opacidad y borde punteado (no clickeable)
      return `${baseClasses} text-white bg-green-500/60 border-2 border-dashed border-green-400 cursor-not-allowed active:scale-100`;
    }
    
    if (isDateAvailable(date)) {
      // Día que está HABILITADO: Verde sólido
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
          ...
          {getDaysInMonth.map((date) => (
            <div key={date.toISOString()} className="relative flex justify-center p-1">
              ...
              >
                ...
                {/* Indicador de "Hoy" */}
                {isToday(date) && (
                  <span className="absolute -top-1.5 -right-1.5 text-[7px] sm:text-[9px] px-1.5 py-0.1 rounded-full bg-pink-500 text-white border border-pink-600 shadow-sm">Hoy</span>
                )}
                
                {/* Indicador de día clickeable */}
                {isSelectable(date) && !isToday(date) && (
        ...
        <div className="mt-4 p-4 bg-gray-50 rounded-lg border">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Leyenda de colores:</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 text-xs">
            ...
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-green-500/60 rounded border-2 border-dashed border-green-400"></div>
              <span className="text-gray-700">Habilitado (días anteriores)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[7px] sm:text-[9px] px-1.5 py-0.5 rounded-full bg-pink-500 text-white border border-pink-600 shadow-sm">Hoy</span>
