  ...
  try {
    ...
    try {
      await emailService.sendNewReviewNotification(
        env.EMAIL_USER, // Enviar al email del administrador configurado
        {
          id: review.id,
...
export const approveReview = async (req: Request, res: Response) => {
  try {
    ...
    if (sendNotification === true && req.body.clientEmail) {
      try {
        await emailService.sendReviewApprovedNotification(
...
export const getUnreadReviewsCount = async (_req: Request, res: Response) => {
  try {
...
export const getPendingReviewsCount = async (_req: Request, res: Response) => {
  try {
    const count = await prisma.review.count({
      where: {
        isApproved: false
