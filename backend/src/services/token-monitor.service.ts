...
export class TokenMonitorService {
  ...
  private readonly CHECK_INTERVAL = 6 * 60 * 60 * 1000; // 6 horas
   ...
   */
  private async checkGoogleDriveToken() {
    try {
      ...
      if (errorMessage.includes('invalid_grant')) {
        ...
        try {
          await emailService.sendTokenRenewalNotification(
            ...
            'gmail',
            'error',
            'El token de Gmail ha expirado y necesita reautorización manual'
          );
        } catch (emailError) {
          logger.warn('Error enviando notificación de token expirado:', emailError);
        }
