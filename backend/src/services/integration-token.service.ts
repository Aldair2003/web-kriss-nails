...
export class IntegrationTokenService {
   ...
   */
  async getToken(provider: string): Promise<{
    refreshToken: string;
    accessToken?: string;
    expiresAt?: Date;
    needsAuth: boolean;
   ...
   */
  async markTokenNeedsAuth(provider: string): Promise<void> {
    try {
   ...
   */
  async updateTokenCheck(provider: string, isValid: boolean): Promise<void> {
    try {
      await prisma.integrationToken.update({
        where: { provider },
        data: {
   ...
   */
  async getTokensNeedingAuth(): Promise<string[]> {
    try {
